...
Item {
  ...
  function runScreensaverProcess(force) {
    ...
    var command = [
      "bash",
      "-lc",
      "[[ $(omarchy-shell lock isLocked 2>/dev/null) == \"true\" ]] || exec \"$1\" \"${@:2}\"",
      "omarchy-idle-screensaver",
      root.screensaverLauncherPath
    ]
    if (force) command.push("force")

    logEvent("process-start", "screensaver " + root.screensaverLauncherPath + (force ? " force" : ""))
    screensaverProcess.command = command
  ...
  function launchScreensaver() {
    ...
    runScreensaverProcess(false)
  }

  function launchScreensaverManually() {
    return runScreensaverProcess(true) ? "started" : "busy"
  }
  ...
  IpcHandler {
    ...
    function screensaver(): string {
      return root.launchScreensaverManually()
    }
  }
}
