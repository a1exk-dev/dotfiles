...
Guidance: Identify a Managed wallpaper with the full lowercase SHA-256 digest of its exact, unmodified bytes and append the canonical extension detected from supported image content. Accept JPEG, PNG, GIF, BMP, or WebP only after warning-free complete decoding of all image data or animation frames. Reuse exact duplicates and fail closed if equal digests have different bytes. Keep tracked `wallpapers/inbox/.gitkeep` because ignored candidate siblings otherwise leave the Wallpaper inbox absent from fresh clones; reserve and hide `.gitkeep` from intake-facing Inspect and Add scans, but retain it in transaction snapshots as stale-plan evidence. Allow unrelated worktree changes, but validate the complete Wallpaper library and revalidate confirmed operation paths before mutation. Serialize Add, Move, and Remove with recoverable transaction evidence that restores prior assignments and Intake images after failure or interruption. Add must create one path-scoped Git commit for assignment paths absent from HEAD, use `Add managed wallpaper <digest-prefix>` as the subject, bypass ordinary hooks, preserve unrelated staged work, and skip the commit when every assignment is already exact in HEAD. Keep automatic-commit tests to basic success, committed no-op, and ordinary failure cases; add deeper safety coverage only for a reproduced defect. Delete an Intake image only after the complete Managed wallpaper and every requested Theme assignment exist in repository state and the stored bytes match the intake.

Reason: Content identity avoids source-name collisions and remains stable across Theme assignments. Complete decoding keeps unreadable assets out of Git, while transactional restoration and post-verification deletion preserve recoverable maintainer input. Path-scoped commits keep ingested assignments in Git without including unrelated staged work. Basic coverage keeps the small-script test budget under five minutes without speculative safety matrices.
...
Guidance: Use `make` and guided setup as the intended human workflow. Keep every operation available as a standalone wizard action. The current sequence is prerequisite preparation, pinned global skill installation, application cleanup, optional application installation, Stow package application, Wallpaper library deployment, optional shared Brave policy application, optional Laptop power policy application, and optional `Install OBS set`. Phase four reuses the standalone optional application installer and starts its selection empty. Phase six calls the same wallpaper Apply function as the standalone action. A declined plan, empty unowned library, or exact no-op is a successful skip. Phase seven calls the standalone Brave Apply function and skips successfully when no supported browser is installed or the human declines. Phase eight calls the standalone Laptop power policy Apply function and skips successfully when battery or hibernation eligibility is unavailable or the human declines. Phase nine asks first, then calls `install_obs_set --guided`, which skips successfully when the human declines, OBS is running, or no set fits the machine. Use Gum with a Bash fallback. Start Stow package selection with no package selected. Continue after a skipped nonessential phase. Stop after an operational failure and name the related standalone action for recovery. Preserve public action preselection and noninteractive operation functions for Make targets, agents, scripts, and tests, but keep them out of normal human instructions.
...
Guidance: Install every skill exposed by the source installers at the revisions pinned in the manifest, except each Repository skill: pass the remaining names through the Skills CLI `--skill` option, and remove an existing global copy of a Repository skill through the official `skills remove` after approval. Delegate draft exclusion, skill discovery, and supporting-file installation to each source's documented installer; this repository adds only manifest pinning, complete recursive difference preview, confirmation, comprehensive pre-mutation backup, and verification. Before a mutating source installer runs, back up every existing skill it can rewrite, including currently unchanged skills, then verify its output against the approved preview. The skill update operation previews upstream and installed-skill differences, then atomically updates the manifest and installed skills after one approval. A failed update restores both the prior manifest and skill backups.

Reason: Future agents need reproducible repository skills without silently overwriting global customizations.

## Declare every global skill source by hand

Applies when: A global agent skill is installed or found under `~/.agents/skills/` without an owning source in `skills.json`.

Guidance: Add the source to `skills.json` by hand, pinned to a full revision with the count the pinned Skills CLI discovers, and add its URL and name to the approved-source allowlist in `validate_skill_manifest`. Then update the harness skill fakes, the manifest test, and the sources table in `docs/agent-setup.md`. The update action only refreshes sources already listed. Compare `~/.local/state/skills/.skill-lock.json` against `skills.json` to find undeclared installs.

Reason: Neither the wizard nor the Skills CLI adds sources. The CLI records global installs only in its own lock file, and the wizard backs up and restores undeclared skills as unrelated without reporting them, so hand installs such as `npx skills add ... -g` or the CLI's `find-skills` prompt never reach a new machine.
...
Applies when: A Stow package links files into `~/.config/omarchy/hooks/<name>.d/`.
