...
BarWidget {
	id: root
	moduleName: "dotfiles.keyboard-layout"
	...
	WidgetButton {
		id: button
		readonly property bool themedFlagVisible: root.layoutFull === "English (US)" || root.layoutFull === "Russian"
		anchors.fill: parent
		bar: root.bar
		text: themedFlagVisible ? "" : root.layoutLabel
		fontSize: Style.font.caption
		fixedWidth: themedFlagVisible ? Style.bar.iconSlot : -1
		hasVisualContent: themedFlagVisible || root.layoutLabel !== ""
		horizontalMargin: 6
		...
		Item {
			...
			width: 12
			height: 8
			visible: button.themedFlagVisible
			...
			Item {
				anchors.fill: parent
				visible: root.layoutFull === "English (US)"
			...
			Item {
				anchors.fill: parent
				visible: root.layoutFull === "Russian"

				Rectangle { x: 1; y: 1; width: 10; height: 2; color: button.foreground; opacity: 0.25 }
				Rectangle { x: 1; y: 3; width: 10; height: 2; color: button.foreground; opacity: 0.55 }
				Rectangle { x: 1; y: 5; width: 10; height: 2; color: button.foreground }
			}
		}
	}
}
