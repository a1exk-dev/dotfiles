...
set -e
run_test_parallel test_public_interface_and_fixed_paths 'Brave public interface and production paths are fixed'
run_test_parallel test_canonical_policy_rejects_every_shape_family 'canonical policy rejects missing, changed, extra, and mistyped values'
run_test_parallel test_duplicate_aware_parser_covers_policy_foreign_and_receipts 'duplicate-aware parser covers canonical, foreign, and receipt JSON'
run_test_parallel test_no_follow_helper_primitives_reject_symlink_inputs 'no-follow helper primitives reject symlink inputs without disclosure'
run_test_parallel test_status_is_read_only_for_clean_absence 'status is read-only for clean absence'
run_test_parallel test_no_browser_apply_returns_11_before_system_inspection 'no-browser apply returns typed outcome 11 without mutation'
run_test_parallel test_consumer_matrix_and_version_warning 'consumer matrix and package baseline warning are reported'
run_test_parallel test_unsupported_provider_matrix_blocks_before_mutation 'unsupported provider matrix blocks apply before mutation'
run_test_parallel test_status_classification_matrix 'status classifies exact, drifted, unowned, stale, malformed, pending, and recovery states'
run_test_parallel test_destination_no_follow_and_metadata_matrix 'destination no-follow and metadata safety matrix blocks apply'
...
run_test_parallel test_backup_pending_and_stage_failures_precede_or_rollback_mutation 'backup, pending, and stage failures preserve or roll back state'
run_test_parallel test_single_install_staging_replaces_links_without_touching_referents 'single-install apply and restore staging preserve file and directory link referents'
run_test_parallel test_receipt_publication_is_atomic_and_directory_safe 'receipt publication is atomic and directory-safe'
run_test_parallel test_incomplete_apply_preview_blocks_confirmation_and_mutation 'incomplete apply preview blocks confirmation and mutation'
run_test_parallel test_failures_after_publication_restore_prior_state 'post-publication failures restore prior target and receipt state'
run_test_parallel test_rollback_failure_records_recovery_and_reconciles_only 'rollback failure records recovery and recovery does not continue ordinary apply'
run_test_parallel test_interrupted_restore_rechecks_every_confirmed_state_family 'interrupted restore rechecks receipts, paths, foreign policy, target, and backups'
run_test_parallel test_interrupted_apply_completion_is_idempotently_reconciled 'interrupted apply completion reconciles idempotently'
run_test_parallel test_remove_noop_unowned_and_stale_receipt_behaviors 'remove handles no-op, unowned target, and stale receipt safely'
run_test_parallel test_safe_remove_restores_color_only_directory_and_preserves_canaries 'safe remove restores metadata and preserves all canaries'
run_test_parallel test_remove_ignores_source_provider_and_foreign_errors_but_retains_hardening 'remove survives source, provider, and foreign errors while retaining hardening'
run_test_parallel test_remove_originally_absent_directory_and_receipt_clear_rerun 'remove handles originally absent directory and receipt-clear rerun'
run_test_parallel test_remove_blocks_special_target_and_malformed_receipt 'remove blocks special targets and malformed receipts'
run_test_parallel test_manage_submenu_back_and_public_argument_rejection 'manage submenu and public argument rejection are exact'
run_test_parallel test_state_and_backup_no_follow_safety 'state and backup roots enforce absolute protected no-follow paths'
run_test_parallel test_state_root_creation_race_rejects_symlink_without_chmodding_referent 'state-root creation race rejects symlink without chmodding its referent'
run_test_parallel test_interrupted_remove_rejects_symlinked_transaction_backup_parent 'interrupted remove rejects a symlinked transaction backup parent'
run_test_parallel test_interrupted_remove_with_intact_prior_state_only_clears_pending 'intact interrupted remove clears only pending state'
run_test_parallel test_partial_system_mutations_roll_back_or_reconcile_monotonically 'partial system mutations roll back or reconcile monotonically'
run_test_parallel test_same_metadata_managed_directory_replacement_fails_and_rolls_back 'same-metadata managed-directory replacement fails and rolls back'
run_test_parallel test_target_backup_symlink_race_reads_and_retains_no_referent_bytes 'target backup symlink race reads and retains no referent bytes'
run_test_parallel test_removal_finalization_rejects_false_success_operations 'removal finalization rejects false-success privileged operations'
run_test_parallel test_ordinary_stale_removal_finalizes_directory_safely 'ordinary stale removal restores safely or retains hardening'
run_test_parallel test_mutating_recovery_shows_omarchy_mismatch_before_one_confirmation 'mutating recovery shows Omarchy mismatch before one confirmation'
run_test_parallel test_state_only_interrupted_remove_displays_omarchy_without_privilege 'state-only interrupted Remove displays Omarchy context without privilege'
run_test_parallel test_automatic_recovery_cleanup_checks_omarchy_before_receipt_mutation 'automatic recovery cleanup checks Omarchy before receipt mutation'
run_test_parallel test_recovery_outcomes_are_caller_aware 'recovery outcomes are exact and caller-aware'
run_test_parallel test_status_distinguishes_installed_consumer_parent_drift 'status distinguishes installed-consumer parent drift from final-browser absence'
run_test_parallel test_cross_filesystem_publish_failure_preserves_target_and_cleans_transaction 'cross-filesystem no-copy publication failure preserves target and cleans transaction state'
run_test_parallel test_empty_regular_target_is_statused_repaired_and_removed_as_regular 'empty regular target is statused, repaired, and removed as a regular file'
run_test_parallel test_unreadable_receipt_owned_target_blocks_status_apply_and_remove 'unreadable receipt-owned target blocks status, Apply, and Remove without mutation'
run_test_parallel test_remove_clears_state_while_hardened_and_rejects_final_replacement 'Remove clears state under hardening and rejects finalization-time replacement'
run_test_parallel test_brave_documentation_contract_and_relative_links 'Brave documentation contract and relative links are structurally complete'
finish_tests
