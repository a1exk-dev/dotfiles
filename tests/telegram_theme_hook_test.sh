...
test_hook_fails_closed_on_exact_package_version_mismatch() {
	...
	for mismatch in omarchy telegram; do
		...
		assert_contains "$COMMAND_OUTPUT$(<"$TELEGRAM_STATUS")" 'omarchy 4.0.x' 'compatibility error should name the supported Omarchy series' || return 1
		assert_contains "$COMMAND_OUTPUT$(<"$TELEGRAM_STATUS")" 'telegram-desktop 7.2.x' 'compatibility error should name the supported Telegram series' || return 1
